...
{
    public partial class FProductGrouping : Form
    {
        ...
        private void cbbPrd_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    db.SetSubModel(ref cbModel, cbbFPlant.SelectedItem.ToString(), cbbPrd.SelectedItem.ToString());
        ...
        void DisplayData()
        {
            try
            {
                ...
                if (cbbFilter.SelectedIndex >= 0)
                {
                    ...
                    dgvReport.DataSource = db.SelectTables("MMProductGrouping", "Plant, Product,SubModel,GroupName, UpdateBy, UpdateDate", field + " LIKE " + cri + " ORDER BY UpdateDate DESC ");
                    ...
                    dgvReport.Columns["GroupName"].Width = 300;
                    dgvReport.Columns["UpdateBy"].Width = 200;                    
                    dgvReport.Columns["UpdateDate"].Width = 500;
        ...
        void SaveNewRecord()
        {
            ...
            try
            {
                ...
                sql = "INSERT INTO MMProductGrouping (Plant, Product, SubModel, GroupName,UpdateBy,UpdateDate) VALUES " +
                    "(" +
                ...
                MessageBox.Show("Product Grouping has been added!", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ViewMode();
            }
            catch (Exception ex)
            {
        ...
        void SaveEditedRecord()
        {
            ...
            try
            {
                ...
                MessageBox.Show("Product Grouping has been updated!", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ViewMode();
            }
            catch (Exception ex)
            {
        ...
        void DisplayValue()
        {
            try
            {
                if (dgvReport.SelectedRows.Count > 0)
                {
                    ...
                    txtLineDesc.Text = dgvReport.SelectedRows[0].Cells["GroupName"].Value.ToString();
        ...
        void DeleteRecord()
        {
            ...
            {
                conn = db.GetConnString();
                sql = "DELETE MMProductGrouping " +
                    " WHERE " +
                    " Plant='" + dgvReport.SelectedRows[0].Cells["Plant"].Value.ToString() + "' AND " +
                    " Product='" + dgvReport.SelectedRows[0].Cells["Product"].Value.ToString() + "' AND " +
                    " Priority='" + dgvReport.SelectedRows[0].Cells["SubModel"].Value.ToString() + "'";

                cmd = new SqlCommand(sql, conn);
                cmd.ExecuteNonQuery();

                MessageBox.Show("Product Grouping has been removed!", "", MessageBoxButtons.OK, MessageBoxIcon.Information);
                ViewMode();
            }
            catch (Exception ex)
            {
